...
func (m *MockService) Authenticate(authToken string) (*userModel.User, *model.Keystore, error) {
	args := m.Called(authToken)
	if args.Get(0) == nil || args.Get(1) == nil {
		return nil, nil, args.Error(1)
	}
	return args.Get(0).(*userModel.User), args.Get(1).(*model.Keystore), args.Error(1)
}

func (m *MockService) Authorize(user *userModel.User, roles ...string) error {
